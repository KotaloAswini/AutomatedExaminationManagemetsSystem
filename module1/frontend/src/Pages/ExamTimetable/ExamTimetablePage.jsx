...
function ExamTimetablePage() {
    ...
    const handleDeleteExam = useCallback((id) => {
        showErrorConfirm('⚠️ Remove this exam from view?', () => {
            // UI-only delete: remove from local state without API call
            setExams(prev => prev.filter(exam => exam.id !== id));
            showSuccess('Exam removed from view');
        });
    }, [showErrorConfirm, showSuccess]);
